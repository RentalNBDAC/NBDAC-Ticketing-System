...
export const validateField = (name: string, value: string): string[] => {
  ...
  if (!sanitizedValue) return errors; // If not required and empty, skip other validations
  
  // Check for invalid characters (except for websiteUrl which has special handling)
  if (name !== 'websiteUrl' && !hasValidCharacters(sanitizedValue)) {
    errors.push('Mengandungi aksara yang tidak dibenarkan');
  }
  ...
  switch (name) {
    ...
    case 'websiteUrl':
      // Check for valid characters including newlines and URLs
      const websiteValidPattern = /^[a-zA-Z0-9\s\-_.,:;()@&+\n\r\u00C0-\u017F\u0100-\u024F\/]*$/;
